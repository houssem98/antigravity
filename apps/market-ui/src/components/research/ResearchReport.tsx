                ...
                <div className="max-w-[1120px] mx-auto px-8 py-6">
                    ...
                    {(report.metadata.confidence || report.metadata.template || report.metadata.verification || report.metadata.claimAudit || report.metadata.citationDensity || report.metadata.factInference || report.metadata.sectionFanout || report.metadata.contextualRetrieval || report.metadata.distillation || report.metadata.revisions || report.metadata.injectionDefense || report.metadata.readers || report.metadata.recency || report.metadata.workflow || report.metadata.entailment || report.metadata.limitations || report.metadata.cacheHit || report.metadata.hitl || report.metadata.budget) && (
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                            ...
                            {report.metadata.cacheHit && (() => {
                                const ch = report.metadata.cacheHit!;
                                const ageSec = Math.round(ch.ageMs / 1000);
                                const ttlSec = Math.round(ch.ttlMs / 1000);
                                const tip = `Session cache hit: this report was generated ${ageSec}s ago. Cache expires after ${ttlSec}s (${Math.round(ttlSec / 60)} min). Re-run the exact same query+model+workflow combo within TTL returns the cached copy instantly.`;
                                return (
                                    <div className="flex items-center gap-1.5 px-2 py-1 rounded-md text-[11px]"
                                        style={{ background: 'rgba(14, 165, 233, 0.12)', color: '#7DD3FC', border: '1px solid rgba(14, 165, 233, 0.3)' }}
                                        title={tip}>
                                        <Sparkles className="w-3 h-3" />
                                        <span>Cached · {ageSec}s old</span>
                                    </div>
                                );
                            })()}
                            ...
                            {report.metadata.workflow && (() => {
                                const w = report.metadata.workflow!;
                                const tip = `Agentic workflow: "${w.label}" preset pinned the report template to ${w.template} and injected ${w.anglesInjected} research angle${w.anglesInjected === 1 ? '' : 's'} and ${w.metricsInjected} metric${w.metricsInjected === 1 ? '' : 's'} into the blueprint beyond what the Chief Analyst inferred.`;
                                return (
                                    <div className="flex items-center gap-1.5 px-2 py-1 rounded-md text-[11px]"
                                        style={{ background: 'rgba(99, 102, 241, 0.12)', color: '#C7D2FE', border: '1px solid rgba(99, 102, 241, 0.3)' }}
                                        title={tip}>
                                        <Sparkles className="w-3 h-3" />
                                        <span>Workflow: {w.label}</span>
                                    </div>
                                );
                            })()}
